...
export function Leadership() {
  ...
  return (
    <section
      id="leadership"
      ref={ref}
      className="py-20 px-4 bg-gradient-to-br from-fuchsia-50 via-purple-50 to-violet-50 dark:from-slate-950 dark:via-slate-900 dark:to-fuchsia-950/20"
    >
      <div className="max-w-6xl mx-auto">
        <motion.h2
          className="text-4xl md:text-5xl font-bold mb-4 text-balance bg-gradient-to-r from-fuchsia-600 to-purple-600 dark:from-fuchsia-400 dark:to-purple-400 bg-clip-text text-transparent"
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {positions.map((position, index) => {
            ...
            return (
              ...
              >
                <Card className="p-6 bg-white/70 dark:bg-slate-900/50 border border-slate-200/50 dark:border-slate-800/80 hover:border-fuchsia-500/30 dark:hover:border-fuchsia-500/30 hover:shadow-xl backdrop-blur-md transition-all h-full">
                  <div className="flex items-start gap-4 mb-4">
                    <div className="p-3 rounded-lg bg-fuchsia-600 text-white dark:bg-fuchsia-500 shadow-md shadow-fuchsia-500/20">
                      <Icon className="w-6 h-6" />
                    </div>
                    <div className="flex-1">
                      <h3 className="text-xl font-bold mb-1 text-fuchsia-900 dark:text-fuchsia-300">{position.title}</h3>
                      <p className="text-sm text-fuchsia-600 dark:text-fuchsia-400 font-semibold">{position.period}</p>
                      <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 font-medium">{position.description}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      {position.highlights.map((highlight, i) => (
                        <li key={i} className="flex gap-3 text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
                          <span className="text-fuchsia-600 dark:text-fuchsia-400 mt-0.5 font-bold">•</span>
                          <span>{highlight}</span>
                        </li>
                      ))}
                    </ul>
                    {position.highlights.length > 2 && (
                      <Button
                        ...
                        className="mt-2 text-fuchsia-600 dark:text-fuchsia-400 hover:text-fuchsia-700 dark:hover:text-fuchsia-300 hover:bg-fuchsia-50 dark:hover:bg-fuchsia-950/30"
                      >
                        {expandedIndex === index ? (
                          <>
